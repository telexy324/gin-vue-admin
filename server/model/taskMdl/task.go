...
type Task struct {
	...
	CommandVar      string       `json:"commandVar" gorm:"type:text;column:command_var"`                                  // 结束时间
	TargetId        string       `json:"targetId" gorm:"type:text;column:target_id"`                                      // 结束时间
	NetDiskUser     string       `json:"netDiskUser" gorm:"-"`                                                            // 结束时间
	NetDiskPassword string       `json:"netDiskPassword" gorm:"-"`                                                        // 结束时间
	SetTaskOuterSeq int          `json:"setTaskOuterSeq" gorm:"type:bigint;not null;default:0;column:set_task_outer_seq"` // 结束时间
	SetTaskInnerSeq int          `json:"setTaskInnerSeq" gorm:"type:bigint;not null;default:0;column:set_task_inner_seq"` // 结束时间
	...
	CommandVars  []string `json:"commandVars" gorm:"-"`  // 结束时间
